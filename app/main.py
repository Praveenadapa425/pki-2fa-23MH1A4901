# File: app/main.py

from fastapi import FastAPI, HTTPException, status
from pydantic import BaseModel
from pathlib import Path

# --- Correct Imports for a Packaged App ---
# The '.' tells Python to import from other files in the same 'app' package
from .crypto_utils import load_private_key, decrypt_seed
from .totp_utils import generate_totp_code, verify_totp_code, seconds_remaining_in_period
from .config import SEED_FILE, DATA_DIR

# Initialize the FastAPI application
app = FastAPI(title="PKI 2FA Microservice")


# --- Pydantic Models for Request Body Validation ---
class DecryptSeedRequest(BaseModel):
    encrypted_seed: str


class Verify2FARequest(BaseModel):
    code: str | None = None


# --- Helper Function ---
def get_hex_seed() -> str:
    """Safely reads the decrypted hex seed from the persistent volume."""
    if not SEED_FILE.exists():
        raise HTTPException(status_code=500, detail="Seed not decrypted yet. Please call /decrypt-seed first.")
    
    try:
        return SEED_FILE.read_text(encoding="utf-8").strip()
    except Exception:
        raise HTTPException(status_code=500, detail="Error reading seed file.")


# --- Startup Event ---
@app.on_event("startup")
def on_startup():
    """Ensure the data directory exists when the application starts."""
    DATA_DIR.mkdir(parents=True, exist_ok=True)


# --- API Endpoints ---
@app.get("/health")
def health_check():
    """A simple endpoint to confirm the API is running."""
    return {"status": "ok"}


@app.post("/decrypt-seed")
def decrypt_seed_endpoint(body: DecryptSeedRequest):
    """Decrypts the seed and saves it to the persistent volume."""
    try:
        private_key = load_private_key("student_private.pem")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to load private key: {e}")

    try:
        hex_seed = decrypt_seed(body.encrypted_seed, private_key)
    except Exception:
        raise HTTPException(status_code=500, detail="Decryption failed")

    try:
        SEED_FILE.write_text(hex_seed, encoding="utf-8")
    except Exception:
        raise HTTPException(status_code=500, detail="Failed to persist decrypted seed")

    return {"status": "ok"}


@app.get("/generate-2fa")
def generate_2fa_endpoint():
    """Generates a new 2FA code."""
    hex_seed = get_hex_seed()
    try:
        code = generate_totp_code(hex_seed)
        valid_for = seconds_remaining_in_period(30)
    except Exception:
        raise HTTPException(status_code=500, detail="Failed to generate TOTP code")

    return {"code": code, "valid_for": valid_for}


@app.post("/verify-2fa")
def verify_2fa_endpoint(body: Verify2FARequest):
    """Verifies a provided 2FA code."""
    if body.code is None or not body.code.strip():
        raise HTTPException(status_code=400, detail="Missing code")

    hex_seed = get_hex_seed()
    try:
        is_valid = verify_totp_code(hex_seed, body.code, valid_window=1)
    except Exception:
        raise HTTPException(status_code=500, detail="Failed to verify TOTP code")

    return {"valid": is_valid}